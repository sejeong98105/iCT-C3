...
int batteryGpio[10]={18, 23, 24, 25, 9, 4, 17, 27, 22, 10};
...
int main()
{
	...
	makeTimer("Serial To RingBuffer Timer", &SerialToRingBufferTimerID, 0, 100); // 50 ms
	delay(20);
...
{ 
	...
	its.it_interval.tv_nsec = msec * 1000000;//msec * 1000000; 
...
//	printf("SendCommandCount = %d, fd = %d\n", SendCommandCount, Fdport);
...
enum BufferStatus bufferSetFinder(struct Buffer *buffer, int *length)
{
	...
	*length = 0;
	...
	while(!FindETXFlag) {
		status = bufferFinder(&RingBuffer, ETX1, &lenetx1, len); // offset = len 는 STX1 STX2 제외하고 찾기 시작
		if(status == BUFFER_FIND_FAIL) return BUFFER_FIND_FAIL;
